...
def process_and_display_images(input_path):
    ...
    for i in range(processed_image.width):
        for j in range(processed_image.height):
            if len(original_pixels[i, j]) == 3:
                r, g, b = original_pixels[i, j]
                r, g, b = map(int, (r, g, b))  # Преобразование значений в целые числа, если они не целые
...
btn = tk.Button(root, text="Select Image and Process", command=lambda: process_and_display_images(select_file()))
